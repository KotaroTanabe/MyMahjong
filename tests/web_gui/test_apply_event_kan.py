...
def test_draw_tile_after_kan_does_not_pop_wall() -> None:
    code = (
        ...
        "const drawEvt = {name:'draw_tile', payload:{player_index:0, tile:{suit:'pin',value:1}}};\n"
        "next = applyEvent(next, drawEvt);\n"
        "console.log(next.wall.tiles.length + ':' + next.dead_wall.length + ':' + next.players[0].hand.tiles.length);"
    )
